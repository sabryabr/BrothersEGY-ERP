...
window.showPage = function(pageId, params) {
  ...
  if (G.pendingReturn && G.pendingReturn.destination === pageId) showReturnBanner();
  else hideReturnBanner();
    ...
    try { renders[pageId](); }
      ...
      if (el) el.innerHTML = `
        <div class="empty-state">
          ...
          <button class="btn btn-ghost btn-sm" onclick="renderPage('${pageId}')">🔄 Retry</button>
        </div>`;
    }
  }
};
...
window.renderPageLoading = function(pageId, icon, title) {
  ...
  if (!el) return;
  el.innerHTML = `
    <div class="section-header"><div><h2>${icon} ${title}</h2></div></div>
...
// ============================================================
// FLEET DATA LOADER
// Builds complete car info from sheet columns
// Sets ID = doc ID for booking matching
// ============================================================
...
window.loadFleetData = async function() {
  try {
    ...
    const allCars = snap.docs.map(function(d) {
      ...
      // Always sync ID to document ID
      data.ID = docId;

      // Build English car label from sheet columns if missing
      if (!data.car_label) {
        ...
        if (brand || model) {
          data.car_label = (brand+' '+model+' ('+year+') '+color+(plate?' | '+plate:'')).trim();
        }
      }
      ...
      if (!data.car_label_ar) {
        ...
        const plate   = data.plate || '';
        if (brandAR || modelAR) {
          data.car_label_ar = ((modelAR||brandAR)+' ('+year+') '+colorAR+(plate?' | '+plate:'')).trim();
        }
      }

      // Build plate string if missing
    ...
    const activeSample = G.fleet
      .filter(function(c){ return getCarStatusCategory(c) !== 'archived'; })
      .slice(0, 5)
      .map(function(c){ return { id: c.id, label: c.car_label, plate: c.plate, color: c.Color || c['col_J'] }; });
    console.log('[Fleet] Active sample:', activeSample);
...
window.loadBookingsData = async function() {
  try {
    ...
    // Trigger KPI reload on dashboard
...
// Uses Cairo timezone and correct active/overdue definitions
// ============================================================
...
window.updateDashboardKPIs = function() {
  ...
  // Available cars — truly available (not rented, not overdue, not future)
  const availEl = document.getElementById('kpi-avail');
  if (availEl && G.fleet.length) {
    availEl.textContent = G.fleet.filter(function(c){
  ...
  if ((activeEl || overdueEl) && G.bookings.length) {
    // Active: today is between start and end, not closed
...
function _updateProposalsBadge() {
  ...
  if (!pb) return;
  if (draftCount > 0) { pb.textContent = draftCount; pb.classList.remove('hidden'); }
...
window.initRealtimeListeners = function() {
  ...
  const _taskUnsub = db.collection('tasks').onSnapshot(function(snap) {
    ...
    snap.docs.forEach(function(d) {
      const t = d.data();
      if (t.assigned_to === G.user?.username && (t.status==='pending'||t.status==='inprogress')) myPending++;
    });
    const tb = document.getElementById('badge-tasks');
    if (tb) { if (myPending>0){tb.textContent=myPending;tb.classList.remove('hidden');}else tb.classList.add('hidden'); }
    const active = document.querySelector('.page-section.active');
    if (active && active.id==='page-tasks' && typeof switchTaskTab==='function') {
      clearTimeout(window._taskRefreshTimer);
      window._taskRefreshTimer = setTimeout(function(){ switchTaskTab('mine',document.getElementById('task-tab-mine')); }, 1500);
    }
  });
  ...
  const _approvalUnsub = db.collection('approvals').where('status','==','pending').onSnapshot(function(snap) {
    ...
    const active = document.querySelector('.page-section.active');
    if (active && active.id==='page-approvals' && typeof loadApprovalsList==='function') {
      clearTimeout(window._approvalRefreshTimer);
      window._approvalRefreshTimer = setTimeout(function(){ loadApprovalsList('pending'); }, 1500);
    }
  });
...
// ============================================================
// FLEET DROPDOWNS
// Shows plate, color, branch, ID tag for each car
// ============================================================

window.populateFleetDropdowns = function() {
  initFleetFilterState();

  const booking = G.fleet.filter(function(car){
    return getCarStatusCategory(car) !== 'archived';
  });

  const opts = booking.map(function(car) {
    ...
    const label = info.label || car.car_label || car.id;
    // Rich dropdown label: status dot + model + plate + color + ID
    const richLabel = `${dot} ${label}${info.colorEN?' ['+info.colorEN+']':''}${info.branchCode?' ('+info.branchCode+')':''}`;
...
window.saveFleetFilterState = function() {
  localStorage.setItem('fleet_filter_'+G.user?.username,
    JSON.stringify({ status: FLEET_FILTER.status, branches: FLEET_FILTER.branches }));
};
