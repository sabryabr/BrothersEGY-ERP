...
// Egypt timezone support | Sheets sync string boolean handling
// ============================================================
...
// ============================================================
// ORDER STATUS
// Enhanced: considers payment status for richer classification
// ============================================================
...
// Enhanced order payment status for richer UI badges
window.getOrderPaymentStatus = function(order) {
  const total = getOrderTotal(order);
  const paid  = getOrderPaid(order);
  if (total <= 0) return 'unknown';
  const pct = paid / total;
  if (pct >= 1)    return 'paid';       // fully paid
  if (pct >= 0.5)  return 'partial';    // >50% paid
  if (pct > 0)     return 'deposit';    // some paid
  return 'unpaid';                       // nothing paid
};
...
// ============================================================
// FLEET HELPERS
// Full label building using all available sheet columns
// ============================================================
...
// Build full car info object for display tags
window.getCarInfo = function(car) {
  ...
  // Branch from City or Branch fields
  ...
  return {
    ...
    branchCode, branchName, contractStatus,
    ownerName, monthlyFee,
...
// 4. Recent overdue (≤30 days) → OVERDUE
// 5. Default → AVAILABLE
// ============================================================
...
window.getCarStatusCategory = function(car) {
  ...
  if (carDocId && G.bookings && G.bookings.length) {
    ...
    const carOrders = G.bookings.filter(function(b) {
      const bId = String(b['كود السيارة'] || b['col_D'] || b['D'] || '').trim();
      return bId !== '' && bId === carDocId;
    });
  ...
  // Rule 3 & 4: Future / Overdue from bookings
  if (carDocId && G.bookings && G.bookings.length) {
    const today      = new Date();
    const todayStart = new Date(); todayStart.setHours(0, 0, 0, 0);
...
// ============================================================
// CAR COUNTS — counts only truly available cars
// ============================================================

window.getCarCounts = function() {
  const counts = { available: 0, rented: 0, overdue: 0, future: 0,
  ...
  // Legacy: keep 'active' alias for available
...
window.attachQRAndBarcode = function(ref) {
  ...
  setTimeout(function() {
    ...
    if (qrEl) {
      qrEl.innerHTML = '';
      try {
        new QRCode(qrEl, { text: verifyUrl, width: 80, height: 80,
    ...
    if (bcEl) {
      try {
        JsBarcode('#' + bcId, ref, { format: 'CODE128', lineColor: '#1e40af',
          width: 1.5, height: 35, displayValue: true, fontSize: 8, margin: 2,
          background: '#f8fafc' });
      } catch(e) {}
    }
  }, 300);
    ...
    <div style="display:flex;align-items:center;gap:14px;background:#f8fafc;
      ...
      <div style="flex:1;">
        ...
        <div style="font-size:9px;color:#64748b;">
          Issued by: ${G.user?.username||'-'} | ${new Date().toLocaleString('en-GB',{timeZone:'Africa/Cairo'})}
        </div>
...
window.printModule = function(a4Id) {
  ...
  setTimeout(function() {
    window.print();
    setTimeout(function() { document.getElementById('print-iso')?.remove(); }, 500);
  }, 150);
};
...
window.autoCreateReceipt = async function(orderId, orderNo, amount, currency, clientName, carLabel, branch) {
  ...
  try {
    const snap = await db.collection('receipts')
      .where('branch_code','==',branchCode).where('receipt_date','==',cairoDate).get();
