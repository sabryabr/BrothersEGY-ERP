...
window.loadFleetRadar = async function() {
  ...
  el.innerHTML = `
    <div class="section-header">
      <div>
        <h2>📡 Fleet Radar</h2>
        <p>Real-time Gantt — all cars and bookings. Drag to scroll.</p>
      </div>
      <div style="display:flex;gap:7px;flex-wrap:wrap;align-items:center;">
        ...
        <div style="display:flex;align-items:center;gap:5px;">
          <span style="font-size:10px;color:var(--text3);font-weight:700;
            text-transform:uppercase;letter-spacing:0.08em;">Timeline:</span>
          ${[
            { label:'1M', days:30  },
            { label:'2M', days:60  },
            { label:'3M', days:90  },
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:10px;
      padding:8px 13px;background:var(--surface2);
      border:1px solid var(--border);border-radius:var(--radius);">
      ${[
        { color:'#3b82f6', label:'Active' },
        { color:'#ef4444', label:'Overdue' },
        { grad:'repeating-linear-gradient(45deg,rgba(239,68,68,0.7),rgba(239,68,68,0.7) 3px,rgba(0,0,0,0.6) 3px,rgba(0,0,0,0.6) 6px)', label:'Overdue tail' },
        { color:'#ef4444', label:'Accident' },
        { grad:'repeating-linear-gradient(45deg,#f59e0b,#f59e0b 3px,#111 3px,#111 6px)', label:'Accident tail' },
        { color:'#475569', label:'Closed' },
        { dashed:true,     label:'Future' }
      ].map(l => `
        <div style="display:flex;align-items:center;gap:5px;
          font-size:10px;font-weight:700;">
          <div style="width:26px;height:10px;border-radius:3px;
            ${l.dashed
    ...
    <div id="gantt-summary-bar"
      style="display:flex;gap:12px;padding:6px 12px;
        background:var(--surface2);border-radius:8px;
        margin-bottom:8px;font-size:11px;flex-wrap:wrap;">
      <span>🚗 <strong id="gsb-total">--</strong> cars shown</span>
      <span style="color:var(--success);">🟢 Available: <strong id="gsb-avail">--</strong></span>
      <span style="color:#3b82f6;">🔵 Rented: <strong id="gsb-rented">--</strong></span>
      <span style="color:#ef4444;">🔴 Accident: <strong id="gsb-accident">--</strong></span>
      <span style="color:#f59e0b;">🟡 Maintenance: <strong id="gsb-maint">--</strong></span>
    </div>

    <!-- Gantt container -->
    <div id="gantt-wrap"
      style="overflow:hidden;max-height:calc(100vh - 240px);
        border:1px solid var(--border);border-radius:var(--radius);
...
window.toggleFleetFilter = function(group, key, cbName) {
  if (group === 'status') {
    FLEET_FILTER.status[key] = !FLEET_FILTER.status[key];
...
window.toggleAllStatuses = function(cbName) {
  Object.keys(FLEET_FILTER.status).forEach(k => {
    FLEET_FILTER.status[k] = true;
  });
  saveFleetFilterState();
  refreshFleetFilterPills();
  if (cbName && window[cbName]) window[cbName]();
...
window.resetFleetFilters = function(cbName) {
  FLEET_FILTER.status = {
    ...
    accident: true, maintenance: true
  };
  FLEET_FILTER.search = '';
  const isPriv = ['Admin', 'Executive'].includes(G.user?.role);
  Object.keys(FLEET_FILTER.branches).forEach(b => {
...
window.refreshFleetFilterPills = function() {
  const fs = FLEET_FILTER.status;
  const fb = FLEET_FILTER.branches;
  const allStatusOn = Object.values(fs).every(v => v);
  const map = {
    'fpill-all-status':  allStatusOn    ? 'all-on'          : 'off',
...
window.buildLocationFilterBar = function() {
  ...
  bar.innerHTML =
    ...
    locs.map(loc => `
      <button class="filter-btn-loc${cur === loc.v ? ' active' : ''}"
        data-loc="${loc.v}"
        onclick="setLocationFilter('${loc.v}')"
        style="padding:2px 9px;border-radius:99px;
          border:1px solid ${cur === loc.v
            ? 'var(--accent)' : 'var(--border)'};
          background:${cur === loc.v
            ? 'var(--accent)' : 'var(--glass)'};
...
window.renderGantt = async function() {
  ...
  try {
    ...
    // Pre-index orders by car code for performance
    const ordersByCar = {};
      ...
      <div id="gantt-inner"
        ...
        <div style="display:flex;position:sticky;top:0;z-index:20;
          ...
          <div style="position:relative;width:${totalPx}px;
            min-width:${totalPx}px;height:${HEADER_H}px;
          ...
          <div style="position:sticky;right:0;z-index:22;
            background:var(--surface3);
            width:${ASSIGN_COL}px;min-width:${ASSIGN_COL}px;
            height:${HEADER_H}px;
...
function _buildGanttRows(
  ...
  const todayPxBase = Math.max(
    0,
    Math.floor((today - ganttStart) / 86400000) * DAY_PX
  );
  ...
  activeCars.forEach((car, ci) => {
    ...
    const plateDisplay= car.plate || formatPlate(car);
    ...
    rowsHTML += `
      <div style="display:flex;background:${rowBg};
        ...
        <div style="position:sticky;left:0;z-index:15;
          background:${stickyBg};
          width:${CAR_COL}px;min-width:${CAR_COL}px;
          height:${rowHeight}px;padding:0 10px;
          border-right:1px solid var(--border2);
...
// ---- Build bars for a single car's rows ----
function _buildGanttRowBars(
  ...
  if (rows.length === 0) {
    return `
      <div style="position:relative;width:${totalPx}px;
        ...
        <div style="position:absolute;left:${todayPxBase}px;top:0;
          bottom:0;width:2px;background:rgba(59,130,246,0.5);
  ...
  return rows.map((row, ri) => {
    ...
    barsHTML += `<div style="position:absolute;left:${todayPxBase}px;
      top:0;bottom:0;width:2px;background:rgba(59,130,246,0.5);
    ...
    for (let i = 0; i < totalDays; i++) {
      ...
      if (d.getDay() === 0 || d.getDay() === 6) {
        barsHTML += `<div style="position:absolute;
          left:${i * DAY_PX}px;top:0;bottom:0;
          width:${DAY_PX}px;
    ...
    if (ri === 0) {
      barsHTML += _buildProposalBars(
        carProposals, ganttStart, totalPx, DAY_PX, ROW_H
      );
    }
    ...
    return `
      <div style="display:flex;
        ...
        <div style="position:relative;width:${totalPx}px;
          min-width:${totalPx}px;height:${ROW_H}px;
          flex-shrink:0;overflow:hidden;">
          ${barsHTML}
        </div>
        ${aHTML ? `
          <div style="position:sticky;right:0;
            width:230px;min-width:230px;height:${ROW_H}px;
...
function _buildMonthStripes(ganttStart, totalDays, today, DAY_PX, ROW_H) {
  ...
  while (md < new Date(ganttStart.getTime() + totalDays * 86400000)) {
    ...
    html += `<div style="position:absolute;left:${mLeft}px;top:0;
      width:${mW}px;height:100%;
      background:${isCurM
...
function _buildProposalBars(carProposals, ganttStart, totalPx, DAY_PX, ROW_H) {
  ...
  carProposals.forEach(p => {
    ...
    const pStartDay = Math.floor((pStart - ganttStart) / 86400000);
    const pEndDay   = Math.floor((pEnd   - ganttStart) / 86400000);
    const pStartPx  = Math.max(-DAY_PX, pStartDay * DAY_PX);
    const pEndPx    = Math.min(totalPx + DAY_PX, pEndDay * DAY_PX);
...
function _buildOrderBar(
  ...
  const isFuture   = start > today && !isClosed;
  ...
  const baseEndDay  = Math.floor((baseEndDate - ganttStart) / 86400000);
  const baseEndPx   = Math.min(totalPx + DAY_PX, baseEndDay * DAY_PX);
  ...
  if (isFuture && barW > 0) {
    html += `
      <div style="position:absolute;left:${clampedStartPx}px;top:${barTop}px;
        ...
        background:rgba(59,130,246,0.1);" ${barEvents}>
        <div style="padding:1px 4px;font-size:9px;font-weight:700;
          color:var(--accent);white-space:nowrap;overflow:hidden;
          text-overflow:ellipsis;line-height:${barH}px;">
          ${clientName}
        </div>
      </div>`;
  } else if (barW > 0) {
    html += `
      <div style="position:absolute;left:${clampedStartPx}px;top:${barTop}px;
        ...
        transition:filter 0.12s;" ${barEvents}>
        <div style="padding:1px 4px;font-size:9px;font-weight:700;
...
function _buildAssignmentInfo(lastOrd, today, DAY_PX) {
  ...
  const lov = !lcl && le && today > le &&
              (lst === 'Active' || lst === 'Overdue');
...
window.showGanttTip = function(e, tipKey) {
  ...
  let badge = '';
  if (d.isAccident) badge = `<span style="background:var(--orange);color:#fff;padding:2px 8px;border-radius:99px;font-size:10px;font-weight:800;">🚨 ACCIDENT</span>`;
  else if (d.isOverdue) badge = `<span style="background:var(--danger);color:#fff;padding:2px 8px;border-radius:99px;font-size:10px;font-weight:800;">⚠️ OVERDUE</span>`;
  else if (d.isClosed)  badge = `<span style="background:#475569;color:#fff;padding:2px 8px;border-radius:99px;font-size:10px;font-weight:800;">✅ CLOSED</span>`;
  else if (d.isFuture)  badge = `<span style="background:var(--accent);color:#fff;padding:2px 8px;border-radius:99px;font-size:10px;font-weight:800;">📅 UPCOMING</span>`;
  else                  badge = `<span style="background:var(--success);color:#fff;padding:2px 8px;border-radius:99px;font-size:10px;font-weight:800;">🚗 ACTIVE</span>`;
  ...
  t.innerHTML = `
    ...
    <div style="background:${d.debt > 0
        ? 'rgba(239,68,68,0.1)' : 'rgba(34,197,94,0.1)'};
      border:1px solid ${d.debt > 0
  ...
  const ty = Math.min(e.clientY - 10,  window.innerHeight - 460);
...
window.openCarDetailModal = async function(carId) {
  ...
  const car = G.fleet.find(
    c => String(c.ID || c.id) === String(carId)
  );
  ...
  const html = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;
      ...
      <div class="panel">
        ...
        <div style="font-size:11px;display:grid;
          ...
          <span style="font-weight:700;">${car.plate || formatPlate(car)}</span>
    ...
    <div style="margin-bottom:14px;padding:10px 12px;
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
        ...
        <input type="text"
          id="car-location-detail-${car.id}" class="form-input"
  ...
  // Pre-select current location in dropdown
  setTimeout(() => {
...
// ============================================================
// CAR INFO BADGE (used in contract forms)
// ============================================================
...
window.showCarInfoBadge = function(carId, containerId) {
  ...
  const statusLabel =
    ...
    cat === 'maintenance' ? '🟡 Maintenance'   : '⚪ Unknown';

  container.className = 'car-info-badge visible';
  container.innerHTML = [
    { label: 'Plate',    value: car.plate || formatPlate(car) },
    { label: 'Year',     value: car.year  || car['سنة الصنع'] || '-' },
    { label: 'Color',    value: car.Color || car['اللون']      || '-' },
    { label: 'Seats',    value: car['Seat_No'] || car['عدد المقاعد'] || '-' },
...
async function _renderVehicle360Full() {
  ...
  let v360Tab = window._v360Tab || 'active';
                      ...
                      return `
                        <tr style="cursor:pointer;"
                          ...
                          <td><span style="color:var(--accent);
                            font-weight:700;">#${getOrderNo(o)}</span></td>
