...
// English contract (4-page: Contract, Terms, Inspection, Receipts)
// Arabic contract (4-page: same structure, full Arabic legal terms)
// Both match standalone generator reference designs exactly
// ============================================================
...
// ============================================================
// SHARED TERMS (EN) — from contract-en.html reference
// ============================================================
...
// ============================================================
// SHARED TERMS (AR) — from contract-ar.html reference
// ============================================================
...
const TERMS_AR = [
  ...
  { title: 'باقة التأمين المختارة',
    text: '' /* rendered separately with insurance details */ },
  ...
  { title: 'فض المنازعات',
    text: '' /* filled dynamically with branch name */ },
  { title: 'توثيق العقد',
    text: 'حرر هذا العقد من نسختين، بيد كل طرف نسخة للعمل بموجبها.' }
];

const INS_DETAILS_AR = {
  'Basic': `<strong>مسؤولية عالية.</strong> تدفع 100% عن أضرار الزجاج والإطارات والهيكل السفلي.<br>
    <strong>يشمل:</strong> CDW القياسي (أضرار الهيكل فقط مع مبلغ التحمل).<br>
    <strong>يستثني:</strong> جميع الزجاج والإطارات والهيكل السفلي والسحب والمسؤولية تجاه الغير.`,
  'Intermediate': `<strong>مسؤولية مخفضة.</strong> يدفع العميل 70% من تكاليف الإصلاح. تدفع الشركة 30%.<br>
    <strong>يشمل:</strong> تغطية الهيكل السفلي حتى 10 آلاف جنيه مصري.<br>
    <strong>يستثني:</strong> الزجاج والإطارات والأضرار الداخلية.`,
  'Full Protection': `<strong>إعفاء كامل من مبلغ التحمل (Waiver).</strong> يقلل المسؤولية إلى الصفر للأصناف المغطاة.<br>
    <strong>الحدود الأقصى:</strong> السقف (20 ألف)، الزجاج (25 ألف)، الهيكل السفلي (40 ألف)، الإطارات (4 آلاف/إطار).<br>
    <strong>يستثني:</strong> الأضرار الداخلية (الصالون)، التزود بالوقود الخاطئ، أو الممتلكات الشخصية.`,
...
function _kmAllowed(pkg, days) {
  const pkgKMMap = {
    standard: 0, '500': 500, '750': 750,
    '1000': 1000, '1500': 1500, '2000': 2000,
    '3000': 3000, unlimited: 999999
  };
...
function _enTermBlock(title, content) {
  return `<div style="margin-bottom:10px;break-inside:avoid;">
    <strong style="display:block;font-size:8.5pt;color:#04509D;
      ...
      padding-bottom:2px;margin-bottom:3px;">${title}</strong>
...
function _inspectionChecklist(lang) {
  ...
  const chkRow = lbl => `
    <div style="display:flex;align-items:center;justify-content:space-between;
      ...
      <span style="font-weight:bold;font-size:6.5pt;text-transform:uppercase;flex-grow:1;">
        ${lbl}
      </span>
    ...
    ${groups.map(g => `
      <div style="width:48%;">
        <div style="background:#000;color:#fff;font-weight:bold;font-size:7.5pt;
          padding:2px 4px;margin-bottom:3px;">${g.head}</div>
...
function _buildENContractPages(d) {
  ...
  const baseAmt = d.mode === 'Monthly'
    ? d.daily * Math.ceil(d.days/30) : d.base;
  ...
  const page1 = `
    <div style="${_pageStyle()}">
      ...
      <div style="position:relative;z-index:2;height:100%;display:flex;flex-direction:column;">
        ...
        <div style="border:1px solid #000;padding:8px;margin-bottom:10px;position:relative;border-radius:4px;">
          <div style="background:#000;color:white;padding:1px 8px;font-weight:bold;
            font-size:8pt;position:absolute;top:-10px;left:8px;text-transform:uppercase;">
            PARTIES
          </div>
          <table style="width:100%;border-collapse:collapse;font-size:9pt;">
            <tr>
              ...
              <td style="border:1px solid #000;padding:4px;">
                ...
                <b>Passport/ID:</b> ${d.passport||'—'} | <b>License:</b> ${d.license||'—'} |
                <b>Nation:</b> ${d.nationality||'—'}<br>
                <b>Phone:</b> ${d.phone||'—'} | <b>Email:</b> ${d.email||'—'}<br>
                <b>Addr:</b> ${d.address||'—'}<br>
                ${d.addDrName ? `<b>Add. Driver:</b> ${d.addDrName}${d.addDrLic ? ' | <b>Lic:</b> ' + d.addDrLic : ''}` : ''}
              </td>
            </tr>
          </table>
        </div>
        ...
        <div style="border:1px solid #000;padding:8px;margin-bottom:10px;position:relative;border-radius:4px;">
          ...
          <div style="${insColor}padding:5px;text-align:center;font-size:9pt;
          ...
          <div style="background:#eee;padding:5px;text-align:center;font-size:9pt;margin-top:3px;">
            PAID NOW:
            <strong>${Math.round(d.paidNow).toLocaleString()} EGP</strong>
      ...
      <div style="border:1px solid #000;margin-top:5px;padding:3px;">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:8pt;">
          ...
          <tr>
            ...
            <td style="border:1px solid #000;padding:3px;text-align:right;">___________</td>
          </tr>`).join('')}
          <tr style="font-weight:bold;background:#f0f0f0;">
            <td colspan="2" style="border:1px solid #000;padding:3px;text-align:right;">TOTAL DEDUCTIONS:</td>
            <td style="border:1px solid #000;padding:3px;text-align:right;">_________ EGP</td>
          </tr>
        </table>
      </div>
  ...
  const receiptPages = _buildReceiptPages({
    ...
    paidNow:     d.paidNow,
    pending:     d.pending,
    rentPayStr:  d.rentPayStr,
    depHeld:     d.depHeld,
    depPayStr:   d.depPayStr,
    reqDep:      d.reqDep,
    lang:        'en',
    today
  });
      ...
      <div class="panel sticky-panel"
        ...
        <div style="margin-bottom:13px;">
          ...
          <div class="form-grid">
            ...
            <div class="field" id="ar-prev-wrap" style="display:none;">
              ...
              <input type="text" id="ar-prev-con" placeholder="رقم العقد السابق"/>
            </div>
          </div>
        </div>
        ...
        <div style="margin-bottom:13px;">
          ...
          <div class="form-grid" style="margin-top:7px;">
            <div class="field"><label>عداد الخروج</label>
              <input type="number" id="ar-km-out" placeholder="مثال 52747"/></div>
        ...
        <div style="margin-bottom:13px;">
          ...
          <div class="form-grid">
            ...
            <div class="field"><label>اسم سائق إضافي</label>
              <input type="text" id="ar-add-dr-name" placeholder="اختياري"/></div>
            <div class="field"><label>رخصة سائق إضافي</label>
              <input type="text" id="ar-add-dr-lic" placeholder="اختياري"/></div>
          </div>
        </div>
        ...
        <div style="margin-bottom:13px;">
          ...
          <div class="form-grid">
            ...
            <div class="field"><label>تفاصيل مكان الاستلام</label>
              <input type="text" id="ar-ploc-addr" placeholder="العنوان بالتفصيل"/></div>
            ...
            <div class="field"><label>تفاصيل مكان التسليم</label>
              <input type="text" id="ar-dloc-addr" placeholder="العنوان بالتفصيل"/></div>
          </div>
        </div>
        ...
        <div style="margin-bottom:13px;">
          ...
          <div style="font-size:10px;color:var(--text3);margin-bottom:5px;font-weight:700;">
            الإيجار المدفوع الآن
          </div>
          <div class="form-grid g3" style="margin-bottom:7px;">
            <div class="field"><label>ج.م (نقداً)</label>
        ...
        <div style="margin-bottom:13px;">
          ...
          <div class="form-grid">
            <div class="field"><label>باقة التأمين</label>
              <select id="ar-ins-pkg">
                <option value="No Insurance">بدون تأمين (مسؤولية 100%)</option>
                <option value="Basic">أساسي (مسؤولية العميل 100%)</option>
                <option value="Intermediate">متوسط (70% / 30%)</option>
                <option value="Full Protection">حماية كاملة (مسؤولية صفر)</option>
              </select>
            </div>
            <div class="field"><label>باقة الكيلومترات</label>
              <select id="ar-km-pkg" onchange="arCalc()">
                <option value="standard"  data-km="0">قياسي (120 كم/يوم)</option>
                <option value="500"       data-km="500">500 كم إضافي</option>
                <option value="750"       data-km="750">750 كم إضافي</option>
                <option value="1000"      data-km="1000">1000 كم إضافي</option>
                <option value="1500"      data-km="1500">باقة 1500 كم</option>
                <option value="2000"      data-km="2000">باقة 2000 كم</option>
                <option value="3000"      data-km="3000">باقة 3000 كم</option>
                <option value="unlimited" data-km="999999">كيلومتر مفتوح يوميًا</option>
              </select>
            </div>
            <div class="field"><label>تكلفة باقة الكيلومترات (ج.م)</label>
          ...
          <div style="display:flex;gap:13px;margin-top:7px;flex-wrap:wrap;">
            <label style="display:flex;align-items:center;gap:5px;font-size:11px;
              cursor:pointer;background:var(--surface2);padding:7px 12px;
              border-radius:8px;border:1px solid var(--border);">
              <input type="checkbox" id="ar-add-driver"
                onchange="arCalc()" style="width:14px;height:14px;"/>
              سائق إضافي
              <input type="number" id="ar-dr-rate" value="250" oninput="arCalc()"
                style="width:65px;padding:3px 7px;background:var(--surface2);
                  border:1px solid var(--border);border-radius:6px;color:var(--text);"/>
              <span style="font-size:10px;color:var(--text3);">ج.م/يوم</span>
            </label>
            <label style="display:flex;align-items:center;gap:5px;font-size:11px;
              cursor:pointer;background:var(--surface2);padding:7px 12px;
              border-radius:8px;border:1px solid var(--border);">
              <input type="checkbox" id="ar-add-seat"
                onchange="arCalc()" style="width:14px;height:14px;"/>
              كرسي طفل
              <input type="number" id="ar-seat-rate" value="250" oninput="arCalc()"
                style="width:65px;padding:3px 7px;background:var(--surface2);
                  border:1px solid var(--border);border-radius:6px;color:var(--text);"/>
              <span style="font-size:10px;color:var(--text3);">ج.م/يوم</span>
            </label>
        ...
        <div style="background:var(--surface2);border:1px solid var(--border);
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:11px;">
            ...
            <span style="color:var(--text3);">رسوم/إضافات:</span>
          ...
          <div style="border-top:1px solid var(--border);margin-top:7px;padding-top:7px;font-size:11px;">
            <div style="display:flex;justify-content:space-between;margin-bottom:3px;">
              <span style="color:var(--text3);">التأمين المطلوب:</span>
              <span id="ar-dep-target">£0.00</span>
            </div>
            <div style="display:flex;justify-content:space-between;margin-bottom:3px;">
              <span style="color:var(--text3);">المدفوع الآن:</span>
              <span id="ar-paid-now" style="color:var(--success);">£0.00</span>
            </div>
            <div style="display:flex;justify-content:space-between;">
              <span style="color:var(--text3);">الإيجار المتبقي:</span>
...
window.arCalc = function() {
  ...
  const addDr   = document.getElementById('ar-add-driver')?.checked;
  const drRate  = parseFloat(document.getElementById('ar-dr-rate')?.value)  || 250;
  const addSt   = document.getElementById('ar-add-seat')?.checked;
  const stRate  = parseFloat(document.getElementById('ar-seat-rate')?.value)|| 250;
...
window.generateARContract = async function() {
  ...
  const prevCon    = document.getElementById('ar-prev-con')?.value || '';
  const ref        = await generateDocRef('CN', branchCode);
  ...
  const addDrName= document.getElementById('ar-add-dr-name')?.value || '';
  const addDrLic = document.getElementById('ar-add-dr-lic')?.value  || '';
...
function _buildARContractPages(d) {
  ...
  const poaText = d.poaNum
    ? `بموجب التوكيل رقم (${d.poaNum})${d.poaChar ? ' حرف ' + d.poaChar : ''}${d.poaDate ? ' بتاريخ ' + d.poaDate : ''}`
    : '';

  const insNamesAr = {
    'Basic':          'تأمين أساسي',
    'Intermediate':   'تأمين متوسط',
    'Full Protection':'حماية كاملة',
    'No Insurance':   'بدون تأمين'
  };
  const insNameAr = insNamesAr[d.insPkg] || 'تأمين أساسي';

  const simpleInsTextAr = {
    'Basic':          'تأمين أساسي: مسؤولية العميل 100% عن الأضرار.',
    'Intermediate':   'تأمين متوسط: مسؤولية العميل 70% / تغطية الشركة 30%.',
    'Full Protection':'حماية كاملة: إعفاء كامل من مبلغ التحمل (تطبق الحدود القصوى).',
    'No Insurance':   'بدون تأمين: مسؤولية العميل 100% عن جميع الأضرار والحوادث.'
  };
  ...
  const extHtml = d.agr === 'extension' && d.prevCon
    ? `<div style="font-size:10pt;font-weight:bold;margin-top:3px;">
        (تمديد للعقد رقم ${d.prevCon})</div>` : '';

  const mode = document.getElementById('ar-mode')?.value || 'Daily';
  const mult = mode === 'Monthly' ? Math.max(1, parseFloat((d.days / 30).toFixed(1))) : d.days;
  const rateTxt = mode === 'Monthly'
    ? `${d.daily.toLocaleString('ar-EG')} × ${Math.ceil(d.days/30)} شهر`
    : `${d.daily.toLocaleString('ar-EG')} × ${d.days} يوم`;
  const baseAmt = mode === 'Monthly' ? d.daily * Math.ceil(d.days/30) : d.base;
  ...
  const page1 = `
    <div style="${_pageStyle('rtl')}">
      ...
      <div style="position:relative;z-index:2;height:100%;display:flex;flex-direction:column;">
        ...
        <div style="border:1px solid #000;padding:8px;margin-bottom:8px;position:relative;border-radius:3px;">
          <div style="background:#000;color:white;padding:1px 8px;font-weight:bold;font-size:8pt;
            position:absolute;top:-10px;right:8px;">الأطراف</div>
          <div style="font-size:8.5pt;line-height:1.4;padding-top:5px;">
            <strong>أولاً (المؤجر):</strong>
            شركة الأخوة المتحدون (أي جي) بصفتها وكيلاً عن المالك بإدارة السيارة ${poaText}.<br>
            <strong>ثانياً (المستأجر):</strong>
            السيد/ <strong>${d.fname} ${d.lname}</strong><br>
            جواز سفر / رقم بطاقة الرقم القومي: ${d.passport||'—'} |
            الصادرة من: ${d.nationality||'—'} | هاتف: ${d.phone||'—'}<br>
            الكائن بالعنوان: ${d.address||'—'} | بريد إلكتروني: ${d.email||'—'}<br>
            رخصة قيادة رقم: ${d.license||'—'}
            ${d.addDrName ? `<br><strong>سائق إضافي:</strong> ${d.addDrName}${d.addDrLic ? ' | رخصة: ' + d.addDrLic : ''}` : ''}
          </div>
        </div>

        <div style="font-size:8pt;margin:4px 0;text-align:center;
          background:#eee;padding:3px;border-radius:3px;">
          بعد أن أقر الطرفان بأهليتهما للتصرف والتعاقد قانونياً، اتفقا على ما يلي:
        </div>

        <div style="border:1px solid #000;padding:8px;margin-bottom:8px;position:relative;border-radius:3px;">
          <div style="background:#000;color:white;padding:1px 8px;font-weight:bold;font-size:8pt;
            position:absolute;top:-10px;right:8px;">البند الثاني: بيانات السيارة موضوع التعاقد</div>
          <table style="width:100%;border-collapse:collapse;font-size:8.5pt;margin-top:5px;">
            ...
            <tr>
              <th style="border:1px solid #000;padding:4px;background:#f0f0f0;">موتور رقم</th>
              <td style="border:1px solid #000;padding:4px;">${d.carMotor||'—'}</td>
              <th style="border:1px solid #000;padding:4px;background:#f0f0f0;">شاسيه رقم</th>
              <td style="border:1px solid #000;padding:4px;">${d.carVin||'—'}</td>
        ...
        <div style="border:1px solid #000;padding:8px;margin-bottom:8px;position:relative;border-radius:3px;">
          <div style="background:#000;color:white;padding:1px 8px;font-weight:bold;font-size:8pt;
            position:absolute;top:-10px;right:8px;">البند الثالث والخامس: الفترة والمسافة</div>
          <table style="width:100%;border-collapse:collapse;font-size:8.5pt;margin-top:5px;">
            <tr>
        ...
        <div style="border:1px solid #000;padding:8px;margin-bottom:8px;position:relative;border-radius:3px;">
          <div style="background:#000;color:white;padding:1px 8px;font-weight:bold;font-size:8pt;
            position:absolute;top:-10px;right:8px;">البند الرابع: البيان المالي</div>
          <table style="width:100%;border-collapse:collapse;font-size:8.5pt;margin-top:5px;">
            <thead>
              <tr>
                <th style="border:1px solid #000;padding:4px;background:#f0f0f0;">وصف البند</th>
                <th style="border:1px solid #000;padding:4px;background:#f0f0f0;">السعر / الاحتساب</th>
                <th style="border:1px solid #000;padding:4px;background:#f0f0f0;text-align:left;">المبلغ (ج.م)</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td style="border:1px solid #000;padding:4px;">القيمة الإيجارية للمدة</td>
                <td style="border:1px solid #000;padding:4px;">${rateTxt}</td>
                <td style="border:1px solid #000;padding:4px;text-align:left;">
                  ${Math.round(baseAmt).toLocaleString('ar-EG')}
                </td>
              </tr>
              ${d.kmCostVal > 0 ? `<tr>
                <td style="border:1px solid #000;padding:4px;">باقة الكيلومترات</td>
                <td style="border:1px solid #000;padding:4px;">رسوم الباقة</td>
                <td style="border:1px solid #000;padding:4px;text-align:left;">
                  ${d.kmCostVal.toLocaleString('ar-EG')}
                </td>
              </tr>` : ''}
              ${d.insT > 0 ? `<tr>
                <td style="border:1px solid #000;padding:4px;">رسوم التأمين المختارة</td>
                <td style="border:1px solid #000;padding:4px;">${d.insFee} × ${d.days} يوم</td>
                <td style="border:1px solid #000;padding:4px;text-align:left;">
                  ${Math.round(d.insT).toLocaleString('ar-EG')}
                </td>
              </tr>` : ''}
              ${d.addDr ? `<tr>
                <td style="border:1px solid #000;padding:4px;">سائق إضافي</td>
                <td style="border:1px solid #000;padding:4px;">${d.drRate} × ${d.days} يوم</td>
                <td style="border:1px solid #000;padding:4px;text-align:left;">
                  ${Math.round(d.drT).toLocaleString('ar-EG')}
                </td>
              </tr>` : ''}
              ${d.addSt ? `<tr>
                <td style="border:1px solid #000;padding:4px;">كرسي طفل</td>
                <td style="border:1px solid #000;padding:4px;">${d.stRate} × ${d.days} يوم</td>
                <td style="border:1px solid #000;padding:4px;text-align:left;">
                  ${Math.round(d.stT).toLocaleString('ar-EG')}
                </td>
              </tr>` : ''}
              ${d.delFee + d.pickFee > 0 ? `<tr>
                <td style="border:1px solid #000;padding:4px;">رسوم التوصيل/الاستلام</td>
                <td style="border:1px solid #000;padding:4px;">خدمات لوجستية</td>
                <td style="border:1px solid #000;padding:4px;text-align:left;">
                  ${(d.delFee + d.pickFee).toLocaleString('ar-EG')}
                </td>
              </tr>` : ''}
            </tbody>
            <tfoot>
          ...
          <div style="${insColor}padding:5px;text-align:center;font-size:8.5pt;
            margin-top:4px;border-radius:4px;">
          ...
          <div style="background:#eee;padding:5px;text-align:center;font-size:8.5pt;margin-top:3px;">
            مدفوع الآن:
            <strong>${Math.round(d.paidNow).toLocaleString('ar-EG')} ج.م</strong>
            ${d.rentPayStr ? `<span style="font-size:8pt;"> (${d.rentPayStr})</span>` : ''}
            <br>
            <span style="color:${d.pending > 0 ? '#c0392b' : '#27ae60'};font-weight:bold;">
        ...
        ${d.reqDep > 0 || d.depHeld > 0 ? `
        <div style="border:2px solid orange;padding:5px;margin-bottom:8px;
          ...
          <span style="float:left;font-weight:bold;
            color:${d.depHeld < d.reqDep ? '#c0392b' : '#27ae60'};">
            ${d.depHeld < d.reqDep
              ? Math.round(d.reqDep - d.depHeld).toLocaleString('ar-EG') + ' متبقي'
              : 'محتجز بالكامل'}
          </span>
        </div>` : ''}
  ...
  const termBlock = (title, text) => `
    <div style="margin-bottom:10px;break-inside:avoid;">
      <strong style="display:block;font-size:8.5pt;color:#04509D;
        border-bottom:1px solid #999;padding-bottom:2px;margin-bottom:3px;">
        ${title}
      </strong>
      <div style="font-size:7.5pt;line-height:1.3;">${text}</div>
    </div>`;

  const branchForTerms = BRANCH_AR_NAMES[d.branchCode] || d.branchCode;
    ...
    <div style="${_pageStyle('rtl')}">
      ...
      <div style="position:relative;z-index:2;height:100%;display:flex;flex-direction:column;">
        ...
        <div style="font-size:8.5pt;column-count:2;column-gap:20px;line-height:1.3;flex-grow:1;">
          ${termBlock('1. التمهيد والأطراف', TERMS_AR[0].text)}
          ${termBlock('2. حالة السيارة', TERMS_AR[1].text)}
          ${termBlock('3. المخالفات والحوادث', TERMS_AR[2].text)}
          ${termBlock('4. الأمانة (عارية الاستخدام)', TERMS_AR[3].text)}
          ${termBlock('5. الأنظمة والقوانين', TERMS_AR[4].text)}
          ${termBlock('6. المسؤولية الشخصية', TERMS_AR[5].text)}
          ${termBlock('7. تمديد الإيجار', TERMS_AR[6].text)}
          ${termBlock('8. الإرجاع المبكر', TERMS_AR[7].text)}
          ${termBlock('9. الحوادث والإصلاح', TERMS_AR[8].text)}
          <div style="border:2px solid #e67e22;padding:5px;background:#fff8e1;
            break-inside:avoid;margin-bottom:10px;">
            <strong style="display:block;font-size:8.5pt;color:#e67e22;
              border-bottom:1px solid #e67e22;padding-bottom:2px;margin-bottom:4px;">
              10. باقة التأمين المختارة: ${insNameAr}
            </strong>
            <div style="font-size:7.5pt;line-height:1.3;">${insDetail}</div>
          </div>
          ${termBlock('11. محظورات الاستخدام', TERMS_AR[10].text)}
          ${termBlock('12. براءة الذمة', TERMS_AR[11].text)}
          ${termBlock(`13. فض المنازعات`,
            `تختص محكمة ${branchForTerms} بالنظر في أي نزاع ينشأ حول بنود هذا العقد لا قدر الله.`)}
          ${termBlock('14. توثيق العقد', TERMS_AR[13].text)}
        </div>
        <div style="margin-top:auto;padding:8px;border-top:1px solid #000;
          font-weight:bold;text-align:center;">
          <strong>أقر أنا المستأجر بأنني قرأت وفهمت جميع الشروط والأحكام المذكورة أعلاه وأوافق عليها.</strong>
          <br><br>
  ...
  // ---- PAGE 3: INSPECTION AR with المقر بما فيه ----
  const checklist = _inspectionChecklist('ar');

  const pickupColAR = `
    <div style="width:50%;padding-left:10px;display:flex;flex-direction:column;">
      <div style="background:#eee;font-weight:bold;padding:4px;text-align:center;font-size:8.5pt;
        border-bottom:1px solid #000;margin-bottom:5px;text-transform:uppercase;">
        بيان الاستلام (بدء الإيجار)
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:7pt;margin-bottom:5px;">
        <tr style="background:#eee;">
          <th style="border:1px solid #000;padding:3px;width:50%;">وقت وتاريخ الاستلام</th>
      ...
      <div style="text-align:center;font-size:7pt;font-weight:bold;margin-bottom:3px;">
        الأضرار الموجودة:
      </div>
      <div style="width:100%;aspect-ratio:960/618;display:flex;align-items:center;
        justify-content:center;margin-bottom:2px;">
        <img src="${CAR_IMG}" style="width:100%;height:100%;object-fit:contain;">
      </div>
      <div style="display:flex;align-items:center;gap:5px;justify-content:center;margin-top:2px;">
        <img src="${TIRE_IMG}" style="height:40px;object-fit:contain;">
        <div style="font-size:7pt;"><strong>الإطار:</strong> _______________</div>
      </div>
      ${checklist}
      <div style="margin-top:5px;border:1px solid #ccc;padding:3px;min-height:35px;font-size:7pt;">
        <strong>ملاحظات / إضافات:</strong><br>
        إطار احتياطي [ ] كوريك [ ] طفاية حريق [ ] مثلث [ ]
      </div>
    </div>`;
  ...
  const returnColAR = `
    <div style="width:50%;border-left:1px dashed #000;padding-right:10px;display:flex;flex-direction:column;">
      <div style="background:#eee;font-weight:bold;padding:4px;text-align:center;font-size:8.5pt;
        border-bottom:1px solid #000;margin-bottom:5px;text-transform:uppercase;">
        بيان العودة (نهاية الإيجار)
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:7pt;margin-bottom:5px;">
        <tr style="background:#eee;">
          <th style="border:1px solid #000;padding:3px;width:50%;">وقت وتاريخ العودة</th>
          <td style="border:1px solid #000;padding:3px;">____________________</td>
        </tr>
        <tr style="background:#eee;">
      ...
      <div style="border:1px solid #000;margin-top:5px;padding:3px;">
        <div style="background:#000;color:#fff;font-weight:bold;font-size:7.5pt;
          padding:2px 4px;margin-bottom:3px;">التسوية النهائية</div>
        <table style="width:100%;border-collapse:collapse;font-size:7pt;">
          <tr>
            <th style="border:1px solid #000;padding:3px;">وصف البند</th>
            <th style="border:1px solid #000;padding:3px;">السعر × الكمية</th>
            <th style="border:1px solid #000;padding:3px;">المبلغ</th>
          </tr>
          ${['كم زائد','تأخير','تنظيف/وقود','أضرار/مخالفات'].map(item => `
          <tr>
            <td style="border:1px solid #000;padding:3px;">${item}</td>
            <td style="border:1px solid #000;padding:3px;text-align:center;">___ × ___</td>
            <td style="border:1px solid #000;padding:3px;text-align:left;">________</td>
          </tr>`).join('')}
          <tr style="font-weight:bold;background:#f0f0f0;">
            <td colspan="2" style="border:1px solid #000;padding:3px;text-align:left;">إجمالي المطلوب:</td>
            <td style="border:1px solid #000;padding:3px;text-align:left;">_______ ج.م</td>
          </tr>
        </table>
      </div>
    </div>`;
    ...
    <div style="${_pageStyle('rtl')}">
      ...
      <div style="position:relative;z-index:2;height:100%;display:flex;flex-direction:column;">
        ...
        <div style="margin-bottom:6px;padding:6px;border:1.5px solid #000;
          ...
          بأنني استأجرت السيارة وأنها على مسؤوليتي الشخصية، وأنا مسؤول مسؤولية كاملة
          عن السيارة ابتداءً من تاريخ وساعة استلامها فيما يتعلق بالجوانب المدنية
          والجنائية ومسؤول عن جميع المخالفات والحوادث وجميع الأضرار التي حدثت خلال
          فترة الإيجار. وأقر بأنني قرأت الشروط والأحكام العامة الواردة في العقد وتم
          إخطاري بالعواقب المحتملة من قبل وكيل التأجير، وهذا هو اعتراف بهذا من جهتي.
        </div>

        <!-- Inspection two columns -->
        <div style="display:flex;width:100%;gap:0;flex:1;">
          ${pickupColAR}
          ${returnColAR}
        </div>
        ...
        <div style="margin-top:5px;font-size:8pt;border-top:1px solid #000;padding-top:5px;">
          <div style="font-weight:bold;margin-bottom:5px;color:#e67e22;font-size:7.5pt;">
            هام: إذا لم يتم إغلاق/توقيع تقرير العودة، يحتفظ المستأجر بالمسؤولية الكاملة.
          </div>
          <div style="display:flex;justify-content:space-between;align-items:flex-end;">
            <div style="width:50%;background:#f9f9f9;padding:5px;border:1px solid #000;
              border-radius:3px;font-size:9pt;line-height:1.4;">
              <strong>المقر بما فيه (المستأجر)</strong><br>
              الاسم: ${d.fname} ${d.lname}<br>
              بطاقة رقم: ${d.passport||'—'}<br>
              التوقيع: ____________________
            </div>
            <div style="width:45%;text-align:center;font-size:9pt;">
              <strong>العودة: الموظف</strong><br><br>
...
// ============================================================
// SHARED RECEIPT PAGE BUILDER
// Supports both EN and AR, with monthly loop
// ============================================================
...
function _buildReceiptPages(p) {
  ...
  if (p.reqDep > 0 || p.depHeld > 0) {
    ...
    pages += _singleReceiptPage({
      ...
      detStr:    p.depPayStr || '',
      periodStr: '',
      isDeposit: true, pending: 0, isBlank,
...
function _singleReceiptPage(p) {
  ...
  const singleRec = copyType => `
    <div style="height:48%;border:2px solid #000;padding:15px;
      display:flex;flex-direction:column;box-sizing:border-box;direction:${dir};">
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;
        ...
        <div style="text-align:${p.isAr ? 'left' : 'right'};">
          <img src="${p.qrUrl}" style="height:45px;"><br>
          <div style="font-size:8pt;font-weight:bold;margin-top:3px;">
            ${p.isAr ? 'التاريخ:' : 'DATE:'} ${p.isBlank ? (p.isAr ? '___/___/202_' : '__/__/202_') : p.today}
          </div>
          <div style="font-weight:bold;font-size:9pt;">${p.title}</div>
        </div>
      </div>
      ...
      <div style="display:flex;gap:20px;font-size:9pt;line-height:1.6;
        ...
        <div style="flex:1;">
          <div style="display:flex;justify-content:space-between;border-bottom:1px dotted #eee;">
            <span style="font-weight:bold;font-size:8.5pt;">
              ${p.isAr ? 'استلمنا من' : 'RECEIVED FROM'}
            </span>
            <span style="font-weight:bold;">${p.clientName}</span>
          </div>
          <div style="display:flex;justify-content:space-between;border-bottom:1px dotted #eee;">
            <span style="font-weight:bold;font-size:8.5pt;">
              ${p.isAr ? 'جواز السفر / الهوية' : 'PASSPORT / ID'}
            </span>
        ...
        <div style="flex:1;">
          <div style="display:flex;justify-content:space-between;border-bottom:1px dotted #eee;">
            <span style="font-weight:bold;font-size:8.5pt;">
              ${p.isAr ? 'رقم العقد' : 'CONTRACT NO'}
            </span>
            <span style="font-weight:bold;">${p.ref}</span>
          </div>
          <div style="display:flex;justify-content:space-between;border-bottom:1px dotted #eee;">
            <span style="font-weight:bold;font-size:8.5pt;">
              ${p.isAr ? 'المركبة' : 'VEHICLE'}
            </span>
            <span style="font-size:8pt;">${p.carStr}</span>
          </div>
          ${!p.isDeposit && p.periodStr ? `
          <div style="display:flex;justify-content:space-between;border-bottom:1px dotted #eee;">
            <span style="font-weight:bold;font-size:8.5pt;">
              ${p.isAr ? 'الفترة' : 'PERIOD'}
            </span>
            <span style="font-size:8pt;">${p.periodStr}</span>
          </div>` : ''}
        </div>
      </div>
      ...
      <div style="border:1px solid #ccc;background:#fbfbfb;padding:10px 15px;margin-bottom:5px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px;">
          <span style="font-weight:bold;font-size:10pt;color:#555;text-transform:uppercase;">
            ${p.isAr ? 'المبلغ المستلم' : 'AMOUNT RECEIVED'}
          </span>
          <span style="font-size:${p.isBlank ? '14pt' : '18pt'};font-weight:bold;">
            ${p.isBlank ? (p.isAr ? '_______________ ج.م' : '_______________ EGP') : p.amtStr}
          </span>
        </div>
        <div style="display:flex;justify-content:space-between;">
          <span style="font-weight:bold;font-size:8pt;color:#555;">
            ${p.isAr ? 'تفاصيل الدفع' : 'PAYMENT DETAILS'}
          </span>
        ...
        ${p.isDeposit
          ? `<strong style="color:#c0392b;">
              ${p.isAr
                ...
                : '* Fully refundable if returned in same condition. Deductions apply for damages/fines.'}
             </strong>`
          : (p.pending > 0 && !p.isBlank
              ? `<span style="color:#c0392b;font-weight:bold;">
                  ${p.isAr
                    ? `⚠ الرصيد المتبقي: ${Math.round(p.pending).toLocaleString('ar-EG')} ج.م`
                    : `⚠ OUTSTANDING BALANCE: ${Math.round(p.pending).toLocaleString()} EGP`}
                 </span>`
              : '')}
      </div>
      ...
      <div style="margin-top:auto;display:flex;justify-content:space-between;
        ...
        <div style="text-align:center;font-weight:bold;font-size:9pt;width:40%;">
          ${p.isAr ? 'توقيع المستأجر' : 'LESSEE SIGNATURE'}<br><br>
          ____________________
...
window.printContractWindow = function(containerId) {
  ...
  win.document.write(`<!DOCTYPE html><html><head>
    ...
    <style>
      ...
      .print-btn {
        position:fixed; top:10px; right:10px; background:#04509D; color:white;
