// ============================================================
// modules/receipts.js v4.1
// Receipt generation, management, reprint, verify QR
// ============================================================
...
window.loadReceiptsPage = async function() {
  ...
  el.innerHTML = `
    <div class="section-header">
      ...
      <div style="display:flex;gap:6px;">
        <button class="btn btn-ghost btn-sm rc-main-tab" id="rc-tab-new"
          style="border-color:var(--accent);color:var(--accent);"
          onclick="switchReceiptTab('new',this)">
          ➕ New Receipt
        </button>
        <button class="btn btn-ghost btn-sm rc-main-tab" id="rc-tab-list"
      ...
      <div style="display:grid;grid-template-columns:400px 1fr;
        ...
        <div class="panel sticky-panel"
          ...
          <div style="display:flex;gap:7px;margin-bottom:13px;">
            <div style="flex:1;background:var(--glass);border:1px solid var(--border);
          ...
          <div style="margin-bottom:13px;">
            ...
            <div id="rc-no-contract-warn"
              style="display:none;background:rgba(239,68,68,0.1);
                     ...
                     padding:9px;font-size:11px;color:var(--danger);margin-bottom:9px;">
          ...
          <div style="margin-bottom:13px;">
            ...
            <div class="form-grid">
              ...
              <div class="field">
                ...
                <input type="text" id="rc-client-id" placeholder="Auto-filled"/>
              </div>
              <div class="field">
                <label>Phone</label>
              ...
              <div class="field">
                <label>Vehicle</label>
                <input type="text" id="rc-vehicle" placeholder="Auto-filled"/>
              </div>
              <div class="field">
                <label>Rental Start</label>
                <input type="text" id="rc-start" placeholder="Auto-filled"/>
              </div>
              <div class="field">
                <label>Rental End</label>
                <input type="text" id="rc-end" placeholder="Auto-filled"/>
              </div>
            </div>
          </div>
          ...
          <button class="btn btn-primary btn-full" onclick="generateReceipt()">
            🧾 Generate Receipt
          </button>
          <button class="btn btn-ghost btn-full mt-8" onclick="clearReceiptForm()">
            ✕ Clear Form
          </button>
        </div>
        ...
        <div>
          <div id="rc-output" style="display:none;">
            <div style="display:flex;gap:7px;margin-bottom:11px;flex-wrap:wrap;">
              <button class="btn btn-primary" onclick="printReceiptWindow()">
                🖨️ Print / Save PDF
              </button>
              <button class="btn btn-success" onclick="saveReceiptToERP()">
                💾 Save to ERP
              </button>
          ...
          <div id="rc-placeholder" class="empty-state" style="padding:80px 20px;">
  ...
  if (dateEl) dateEl.value = getCairoDateStr();
...
async function _doRCLookup() {
  ...
  try {
    ...
    if (!data) {
      if (stEl) { stEl.textContent = `No contract found for "${no}"`; stEl.style.color = 'var(--danger)'; }
    ...
    if (clientNo) {
      ...
      const cust = G.customers.find(c => String(c['No.']) === String(clientNo));
      if (cust) set('rc-phone', cust['رقم التليفون'] || '');
    }

    const branchEl = document.getElementById('rc-branch');
    if (branchEl && data['فرع الإصدار']) branchEl.value = data['فرع الإصدار'];
    ...
    const lateDays   = end && getCairoNow() > end && !data.closed
      ? Math.max(0, Math.ceil((getCairoNow() - end) / 86400000)) : 0;
    const latePenalty= lateDays * daily;
    const totalDue2  = totalDue + latePenalty;
    const outstanding2 = Math.max(0, totalDue2 - prevPaid);
      ...
      stEl.innerHTML = `
        <div style="display:flex;align-items:center;gap:9px;padding:9px;
          ...
          <span style="font-size:13px;">
            ${st==='Accident'?'🚨':st==='Overdue'?'⚠️':st==='Closed'?'✅':'🚗'}
          </span>
          <div style="flex:1;">
            <div style="font-size:11px;font-weight:700;color:${stColor};">
              Order #${data['No.']||data.id} — ${data['اسم العميل']||'—'}
              ${st==='Overdue'
                ? `<span style="color:var(--danger);">(${lateDays}d overdue)</span>`
                : ''}
            </div>
            <div style="font-size:10px;color:var(--text3);margin-top:2px;">
              Due: ${fmtMoney(totalDue2)} | Paid: ${fmtMoney(prevPaid)} |
              <strong style="color:${outstanding2>0?'var(--danger)':'var(--success)'};">
                ${outstanding2>0?'Owes: '+fmtMoney(outstanding2):'Fully Paid ✅'}
              </strong>
    ...
    if (stEl) { stEl.textContent = 'Lookup failed: ' + e.message; stEl.style.color = 'var(--danger)'; }
  }
}
...
window.generateReceipt = async function() {
  ...
  if (rcType === 'Deposit Return' && window._rcCurrentBooking) {
    ...
    if (status !== 'Closed' && status !== 'Overdue') {
      if (!confirm(
        'Warning: This order is not yet closed.\n\n' +
  ...
  // ✅ Generate rcRef FIRST — everything else uses it
  const rcRef = await generateDocRef('RC', branchCode);
  ...
  const timestampStr = getCairoTimestamp();

  document.getElementById('rc-ref-preview').textContent = rcRef;
  ...
  // ✅ Save to Firestore with rcRef and verifyUrl
  try {
    const bookingId  = window._rcCurrentBooking?.id || '';
    const receiptData = {
      receipt_ref         : rcRef,
      contract_no         : contractNo,
      order_id            : bookingId,     // ✅ critical for verify lookup
      ...
      receipt_date        : rcDate,
      issued_at           : timestampStr,  // ✅ full timestamp
      type                : rcType,
      amount_egp          : egp,
      amount_usd          : usd,
      amount_eur          : eur,
    ...
    // Use rcRef as the document ID for easy lookup
    await db.collection('receipts').doc(rcRef).set(receiptData);
    ...
    if (window._rcCurrentBooking) {
      ...
      const status  = getOrderStatus(window._rcCurrentBooking);
      if (rcType !== 'Deposit Return' && newTotalPaid >= totalDue &&
          end && getCairoNow() > end && status !== 'Accident' &&
          !updateData['closed']) {
        updateData['closed']      = true;
        updateData['حالة الطلب'] = 'Closed';
        const carId = window._rcCurrentBooking['كود السيارة'];
    ...
    await logAction('GENERATE', 'Receipts',
      `Receipt ${rcRef} | Contract: ${contractNo} | Client: ${clientName} | ` +
      `${fmtMoney(totalThisPayment)} | Outstanding: ${fmtMoney(newOutstanding)}`);

    toast(`Receipt ${rcRef} generated and saved!`, 'success');
  ...
  const html = _buildReceiptA4({
    ref: rcRef, rcTitle, contractNo, rcDate, rcType, branchCode,
    verifyUrl, timestampStr,
    clientName,
  ...
  _attachReceiptQRLocal(rcRef, verifyUrl);
};
...
function _buildReceiptA4(d) {
  ...
  // ✅ verifyUrl comes from d.verifyUrl (already built with rcRef)
  const verifyUrl  = d.verifyUrl || `https://sabryabr.github.io/BrothersEGY-ERP/verify.html?ref=${encodeURIComponent(d.ref)}`;
  ...
  const dateStr    = d.rcDate
    ? new Date(d.rcDate).toLocaleDateString('en-GB') : getCairoTimestamp();
    ...
    <div style="height:48%;border:2px solid #1e40af;padding:13px;
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;
        ...
        <div style="text-align:right;">
          ...
          <div style="font-size:9px;color:#64748b;">
            Issued: <strong>${d.timestampStr || dateStr}</strong>
          </div>
          <div style="font-size:9px;color:#64748b;">
            By: ${G.user?.username || 'Staff'}
          </div>
        </div>
      </div>
      ...
      ${d.totalDue > 0 ? `
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;
                    font-size:8pt;background:#f8fafc;border-radius:4px;
          ...
          <div style="text-align:center;">
            ...
            <div style="font-weight:bold;
              ...
              ${d.newOutstanding>0?'£'+d.newOutstanding.toLocaleString():'✅ PAID'}
            </div>
          </div>
        </div>` : ''}
      ...
      ${d.note ? `
        <div style="font-size:8pt;color:#64748b;margin-bottom:6px;font-style:italic;">
          Note: ${d.note}
        </div>` : ''}
      ...
      <div style="margin-top:auto;display:flex;justify-content:space-between;
        ...
        <div style="text-align:center;width:35%;">
          ${d.autoSign ? `<div style="font-size:22px;color:#1e40af;font-style:italic;
                            margin-bottom:2px;">Brothers EGY</div>`
        ...
        <div style="text-align:center;">
          <div id="${qrElId}" style="width:60px;height:60px;margin:0 auto;"></div>
          <div style="font-size:6pt;color:#64748b;margin-top:2px;">
            Scan to verify<br>${d.ref}
          </div>
        </div>
        <div style="text-align:center;width:35%;">
          ${d.autoSign ? `<div style="font-size:22px;color:#1e40af;font-style:italic;
  ...
  return `
    <div style="width:210mm;min-height:296mm;padding:10mm 14mm;
      ...
      <!-- Footer with barcode and verify URL -->
      <div style="display:flex;align-items:center;gap:12px;background:#f8fafc;
        ...
        <div style="flex:1;font-size:9px;color:#64748b;">
          ...
          <div>Issued: ${d.timestampStr || dateStr}</div>
          <div>
            Verify: sabryabr.github.io/BrothersEGY-ERP/verify.html?ref=${
              encodeURIComponent(d.ref)}
          </div>
        </div>
      </div>
    </div>`;
...
// ============================================================
// QR + BARCODE — local version uses correct verifyUrl
// ============================================================
function _attachReceiptQRLocal(ref, verifyUrl) {
  if (!verifyUrl) {
    verifyUrl = `https://sabryabr.github.io/BrothersEGY-ERP/verify.html?ref=${
...
// Keep the global attachQRAndBarcode working for other modules
window._attachReceiptQR = _attachReceiptQRLocal;
...
window.saveReceiptToERP = async function() {
  ...
  try {
    if (window._rcCurrentBooking) {
      await db.collection('bookings').doc(window._rcCurrentBooking.id).update({
...
window.loadSavedReceipts = async function() {
  ...
  try {
    ...
    _setEl('rcl-kpi-collected', fmtMoney(all.reduce((s,r) => s + (r.total_egp_equiv||0), 0)));
    ...
    wrap.innerHTML = `<div class="empty-state"><p>Could not load receipts: ${e.message}</p></div>`;
  }
};
...
function _renderReceiptsTable(receipts) {
  ...
  wrap.innerHTML = `
    <table class="data-table">
      ...
      <tbody>
        ${receipts.map(r => {
          // ✅ QR link uses receipt_ref (rcRef), not contract_no
          const rRef      = r.receipt_ref || r.qr_ref || r.id;
          const verifyLink= `${verifyBase}?ref=${encodeURIComponent(rRef)}`;
          const issuedStr = r.issued_at || r.receipt_date
            ? (r.issued_at || new Date(r.receipt_date).toLocaleDateString('en-GB'))
              ...
              <td style="color:${(r.outstanding_balance||0)>0
                ...
                <strong>
                  ${(r.outstanding_balance||0) > 0
                    ? fmtMoney(r.outstanding_balance) : '✅ Clear'}
                </strong>
              </td>
              <td style="font-size:11px;">
                ${BRANCH_MAP[r.branch] || r.branch || '—'}
              </td>
              <td style="font-size:11px;">${r.issued_by || r.collected_by || '—'}</td>
              <td>
                <div style="display:flex;gap:3px;flex-wrap:wrap;">
                  <button class="btn btn-primary btn-xs"
                    onclick="reprintReceipt('${r.id}')" title="Reprint">
                    🖨️ Print
                  </button>
                  <a href="${verifyLink}" target="_blank"
                    class="btn btn-ghost btn-xs" title="Verify QR">
                    🔗 Verify
                  </a>
                  ${r.contract_no ? `
...
window.reprintReceipt = async function(receiptId) {
  ...
  // Use reprintFromRecord from orders.js if available and it's a payment receipt
  if (r.receipt_type === 'single_payment' && typeof reprintFromRecord === 'function') {
    reprintFromRecord(r);
    return;
  }
  ...
  setTimeout(async () => {
    ...
    set('rc-date',        r.receipt_date   || getCairoDateStr());
    ...
    const rcRef     = r.receipt_ref || r.qr_ref || r.id;
    // ✅ verifyUrl uses receipt_ref
    const verifyUrl = r.verify_url ||
      `https://sabryabr.github.io/BrothersEGY-ERP/verify.html?ref=${
        encodeURIComponent(rcRef)}`;

    const refPrev = document.getElementById('rc-ref-preview');
    if (refPrev) refPrev.textContent = rcRef;
    ...
    const html = _buildReceiptA4({
      ...
      verifyUrl,
      timestampStr: r.issued_at     || r.receipt_date || '',
    ...
    await logAction('PRINT', 'Receipts', `Reprinted: ${rcRef}`);
  }, 200);
};
