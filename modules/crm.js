// ============================================================
// modules/crm.js
// CRM Hub — customer database, full profile view,
// order history, contract & receipt links,
// add/edit customer, nationality filter, clickable rows
// ============================================================
...
window.loadCRM = async function() {
  ...
  el.innerHTML = `
    <div class="section-header">
      <div>
        <h2>👥 CRM Hub</h2>
        <p>Customer database — search, view, add, edit and track rental history</p>
      </div>
    ...
    <div class="panel" style="margin-bottom:14px;">
      <div style="display:flex;gap:9px;flex-wrap:wrap;align-items:flex-end;">
        ...
        <div>
          <div style="font-size:10px;color:var(--text3);
            margin-bottom:3px;font-weight:700;">NATIONALITY</div>
        ...
        <div>
          <div style="font-size:10px;color:var(--text3);
            margin-bottom:3px;font-weight:700;">SOURCE</div>
...
window.filterCRM = debounce(function() {
  ...
  if (nat    !== 'all') customers = customers.filter(c => (c['رمز الدولة'] || c.col_H) === nat);
  if (source !== 'all') customers = customers.filter(c => c.client_source === source);

  // Sort newest first by numeric CRM ID
  customers.sort((a,b) => {
    ...
    <table class="data-table">
      ...
      <tbody>
        ${customers.slice(0, limit).map(c => {
          ...
          const clientCode= c['No.'] || c.col_A || c.id;
          ...
          return `
            <tr style="cursor:pointer;"
              ...
              <td style="font-size:11px;">
                ${c.client_source
                  ? `<span class="pill pill-accepted"
                      style="font-size:9px;">${c.client_source}</span>`
                  : '—'}
              </td>
              <td style="font-size:11px;">
                ${hasLicense
              ...
              <td onclick="event.stopPropagation()">
                <div style="display:flex;gap:3px;flex-wrap:wrap;">
                  ...
                  <button class="btn btn-primary btn-xs"
                    onclick="viewCustomerOrders('${clientCode}','${name.replace(/'/g,'')}')">
                    📋
                  </button>
                  ${G.user?.role === 'Admin' ? `
                    <button class="btn btn-xs"
                      style="background:var(--danger);color:#fff;"
                      onclick="event.stopPropagation();adminDeleteCustomer('${c.id}')">
                      🗑️
                    </button>` : ''}
                </div>
              </td>
...
// Full profile with financials, order history, receipts
// ============================================================
...
window.openClientProfile = async function(customerId) {
  ...
  const clientOrders = (allOrders.length > 0 ? allOrders : G.bookings)
    .filter(o => String(o['كود العميل']) === String(clientNo));

  // Load receipts
  let receipts = [];
  try {
    const rcSnap = await db.collection('receipts')
      .where('client_id', '==', clientNo).get();
    receipts = rcSnap.docs
  ...
  const html = `
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px;">
      <button class="btn btn-primary btn-sm"
        onclick="closeModal();showPage('proposals');
          setTimeout(()=>prefillProposalClient('${c.id}','${name.replace(/'/g,"\\'")}'),400)">
        📋 New Proposal
      </button>
      <button class="btn btn-ghost btn-sm"
    ...
    <div style="display:flex;align-items:flex-start;gap:14px;padding:13px;
      ...
      <div style="flex:1;">
        ...
        <div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:7px;">
          ...
          <span class="pill pill-closed">${clientOrders.length} Orders Total</span>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;
      ...
      <div class="panel">
        <div style="font-size:10px;font-weight:800;color:var(--accent);
          text-transform:uppercase;margin-bottom:9px;">💰 Financial Summary</div>
        <div style="font-size:11px;display:grid;
          grid-template-columns:auto 1fr;gap:4px 11px;">
          <span style="color:var(--text3);">Total Contracted:</span>
          <span style="font-weight:700;">${fmtMoney(totalContracted)}</span>
          <span style="color:var(--text3);">Total Paid (EGP):</span>
          <span style="font-weight:700;color:var(--success);">
            ${fmtMoney(totalPaid)}
          </span>
          <span style="color:var(--text3);">Outstanding:</span>
          <span style="font-weight:700;
            color:${totalContracted-totalPaid > 0
              ? 'var(--danger)' : 'var(--success)'};">
            ${totalContracted - totalPaid > 0
              ? fmtMoney(totalContracted - totalPaid)
              : '✅ Clear'}
          </span>
    ...
    <div class="panel" style="margin-bottom:13px;">
      <div style="font-size:12px;font-weight:800;margin-bottom:11px;">
        📋 Contracts / Orders (${clientOrders.length})
      </div>
      <div class="table-wrap" style="max-height:220px;overflow-y:auto;">
        <table class="data-table">
          <thead>
            <tr>
              <th>Order #</th><th>Ref</th><th>Car</th>
              <th>Period</th><th>Total</th><th>Status</th><th>Actions</th>
            </tr>
          </thead>
          <tbody>
            ${clientOrders
              ...
              .map(o => {
                ...
                return `
                  <tr style="cursor:pointer;"
                    ...
                    <td style="font-size:9px;color:var(--text3);">
                      ${o.contract_ref || '—'}
                    </td>
                    <td style="font-size:11px;">${carLabel}</td>
                    <td style="font-size:10px;">
                      ${start ? fmtDate(start) : '—'} →<br>
                      ${end   ? fmtDate(end)   : '—'}
                    </td>
                    <td style="white-space:nowrap;">
                      ${fmtMoney(getOrderTotal(o))}
                    </td>
                    <td>
                      <span class="pill pill-${sk}">${st==='Overdue'?'OVERDUE':st}</span>
                    </td>
                    <td onclick="event.stopPropagation()">
                      <div style="display:flex;gap:3px;">
                        <button class="btn btn-ghost btn-xs"
                          onclick="closeModal();openOrderDetail('${o.id}')">👁</button>
                        <button class="btn btn-primary btn-xs"
                          onclick="closeModal();reprintContractFromOrder('${o.id}')">📄</button>
                        <button class="btn btn-ghost btn-xs"
                          onclick="closeModal();reprintReceiptFromOrder('${o.id}')">🧾</button>
                      </div>
                    </td>
                  </tr>`;
              }).join('')}
    ...
    <div class="panel">
      ...
      <div class="table-wrap" style="max-height:200px;overflow-y:auto;">
        <table class="data-table">
          <thead>
            <tr>
              <th>Receipt Ref</th><th>Date</th><th>Contract</th>
            ...
            ${receipts.map(r => {
              ...
              return `
                <tr>
                  <td>
                    <span style="color:var(--accent);font-weight:700;
                      font-size:11px;">${rRef}</span>
                  </td>
                  <td style="font-size:10px;">${r.receipt_date || '—'}</td>
  ...
  await logAction('VIEW', 'CRM Hub', `Viewed profile: ${name} | ID: ${c['No.'] || c.id}`);
};
  ...
  const html = `
    <div class="form-grid">
      ...
      <div class="field">
        ...
        <select id="nc-source">
          ${srcOpts.map(s =>
            `<option value="${s}">${s || '-- How did they find us? --'}</option>`
          ).join('')}
        </select>
      </div>
      <div class="field">
...
window.saveNewCustomer = async function() {
  ...
  try {
    const maxNo  = G.customers.reduce(
      (mx,c) => Math.max(mx, parseInt(c['No.']) || 0), 0
    );
    ...
    const goProposal = confirm(
      `Customer ${fname} ${lname} added!\n\nCreate a proposal for this customer now?`
    );
    if (goProposal) {
      showPage('proposals');
      setTimeout(() => {
        prefillProposalClient(ref.id, `${fname} ${lname}`);
      }, 400);
    } else {
...
window.viewCustomerOrders = function(clientNo, clientName) {
  const orders = (allOrders.length > 0 ? allOrders : G.bookings)
    .filter(o => String(o['كود العميل']) === String(clientNo));
            ...
            .map(o => {
              ...
              return `
                <tr style="cursor:pointer;"
                  ...
                  <td>
                    <span class="pill pill-${sk}">${sl}</span>
                  </td>
                </tr>`;
            }).join('')}
        </tbody>
...
// ============================================================
// PREFILL PROPOSAL FROM CLIENT PROFILE
// ============================================================
...
// ============================================================
// REPRINT CONTRACT / RECEIPT FROM ORDER (helpers)
// ============================================================
...
window.reprintContractFromOrder = function(orderId) {
  ...
  setTimeout(() => {
    ...
    enCalc();
    toast(`Order #${o['No.']} loaded — click Generate Contract to create PDF`,
      'info', 5000);
...
window.reprintReceiptFromOrder = function(orderId) {
  ...
  setTimeout(() => {
    ...
    if (el) {
      el.value = o['No.'] || '';
      window.rcLookupDebounced && rcLookupDebounced();
    }
    toast(`Order #${o['No.']} loaded in receipts — add amount and generate`,
      'info', 5000);
  }, 400);
};
