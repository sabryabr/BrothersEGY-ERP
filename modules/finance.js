...
window.loadIncomeExpenses = async function() {
  ...
  el.innerHTML = `
    ...
    <div style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap;">
      <button class="btn btn-ghost btn-sm ie-tab-btn"
        id="ie-tab-income"
        style="border-color:var(--accent);color:var(--accent);"
        onclick="showIETab('income',this)">
        📥 Income
      </button>
      <button class="btn btn-ghost btn-sm ie-tab-btn"
        id="ie-tab-car"
        onclick="showIETab('car',this)">
        🚗 Car Expenses
      </button>
      <button class="btn btn-ghost btn-sm ie-tab-btn"
        id="ie-tab-gen"
        onclick="showIETab('gen',this)">
        🏢 General Expenses
      </button>
      <button class="btn btn-ghost btn-sm ie-tab-btn"
        id="ie-tab-add"
        onclick="showIETab('add',this)">
        ➕ Add Entry
      </button>
    </div>
...
window.loadIETrendChart = async function() {
  ...
  try {
    ...
    const expByMonth    = {};
    months.forEach(m => {
      incomeByMonth[`${m.year}-${m.month}`] = 0;
      expByMonth[`${m.year}-${m.month}`]    = 0;
    });
    ...
    cSnap.docs.forEach(d => {
      ...
      const key = `${data['سنة']||''}-${String(data['شهر']||'').replace(/^0+/,'')}`;
    ...
    ceSnap.docs.forEach(d => {
      const data = d.data();
      const key  = `${data['paid_year']||data['سنة']||''}-${String(data['شهر']||'').replace(/^0+/,'')}`;
      if (key in expByMonth) {
        expByMonth[key] += parseAmount(data['قيمة المصروف']);
      }
    });
...
window.loadIEBranchBreakdown = async function() {
  ...
  try {
    ...
    const branchAR = {
      'الغردقة': 'HRG', 'الاسكندرية': 'ALX',
      'القاهرة': 'CAI', 'رشيد': 'RSH'
    };
    ...
    cSnap.docs.forEach(d => {
      ...
      const branchArabic = data['موقع الفرع'] || '';
      const branchEn     = data.Branch || '';
      let code = '';
      if (branchAR[branchArabic]) code = branchAR[branchArabic];
      else if (branchEn.toLowerCase().includes('hurghada') || branchEn === 'HRG') code = 'HRG';
      else if (branchEn.toLowerCase().includes('alexandria') || branchEn === 'ALX') code = 'ALX';
      else if (branchEn.toLowerCase().includes('cairo')     || branchEn === 'CAI') code = 'CAI';
      else if (branchEn.toLowerCase().includes('rashid')    || branchEn === 'RSH') code = 'RSH';
    ...
    const branchColors = {
      HRG: 'var(--accent3)', ALX: 'var(--purple)',
      CAI: 'var(--orange)',  RSH: 'var(--success)'
    };
    const branchIcons  = {
      HRG: '🌊', ALX: '🏙', CAI: '🏛', RSH: '⚓'
    };
      ...
      <div class="panel">
        <div style="display:flex;align-items:center;
          ...
          <div style="display:flex;gap:9px;align-items:center;">
            <input type="text" id="ie-income-search"
              placeholder="Search..."
  ...
  return `
    ...
    <div style="padding:9px 11px;font-size:11px;color:var(--text3);
      ...
      <span style="color:var(--success);font-weight:700;">
        Total: ${fmtMoney(rows.reduce((s,r) =>
          s + parseAmount(r['قيمة التحصيل']), 0))}
      </span>
    </div>`;
}
  ...
  try {
    ...
    if (!isPriv) {
      rows = rows.filter(r => {
        ...
        return b.includes(BRANCH_AR[G.user.branch] || G.user.branch);
      });
    }
    if (period.month !== null) {
      rows = rows.filter(r => {
    ...
    pane.innerHTML = `
      <div class="panel">
        <div style="display:flex;align-items:center;
          ...
          <div style="display:flex;gap:9px;align-items:center;">
            <input type="text" id="ie-car-search"
              placeholder="Search..."
              style="padding:5px 10px;background:var(--surface2);
                border:1px solid var(--border);border-radius:7px;
                color:var(--text);font-size:11px;width:160px;"
  ...
  return `
    ...
    <div style="padding:9px 11px;font-size:11px;color:var(--text3);
      ...
      <span style="color:var(--danger);font-weight:700;">
        Total: ${fmtMoney(rows.reduce((s,r) =>
          s + parseAmount(r['قيمة المصروف']), 0))}
      </span>
    </div>`;
}
    ...
    pane.innerHTML = `
      <div class="panel">
        <div style="display:flex;align-items:center;
          ...
          <div style="display:flex;gap:9px;align-items:center;">
            <input type="text" id="ie-gen-search"
              placeholder="Search..."
              style="padding:5px 10px;background:var(--surface2);
                border:1px solid var(--border);border-radius:7px;
                color:var(--text);font-size:11px;width:160px;"
      ...
      <div class="panel">
        ...
        <div style="display:flex;gap:9px;margin-bottom:16px;flex-wrap:wrap;">
          <label style="display:flex;align-items:center;gap:7px;cursor:pointer;
            ...
            id="ie-type-label-income">
            <input type="radio" name="ie-entry-type" value="income"
              onchange="ieTypeChange()"
              style="width:14px;height:14px;accent-color:var(--success);"/>
          ...
          <label style="display:flex;align-items:center;gap:7px;cursor:pointer;
            ...
            border-radius:9px;padding:10px 16px;flex:1;min-width:140px;
            justify-content:center;transition:all 0.2s;"
            id="ie-type-label-gen">
            <input type="radio" name="ie-entry-type" value="gen_expense"
              onchange="ieTypeChange()"
              style="width:14px;height:14px;accent-color:var(--orange);"/>
        ...
        <div class="panel">
          ...
          <div class="form-grid">
            <div class="field">
              ...
              <input type="text" id="ie-inc-order"
                placeholder="e.g. 578 or CN-HRG-..."/>
            </div>
            <div class="field">
            ...
            <div class="field" style="grid-column:1/-1;">
              <label>Statement (بيان التحصيل) *</label>
              <input type="text" id="ie-inc-statement"
                placeholder="What is this income for?"/>
            </div>
            <div class="field">
              <label>Amount (EGP) *</label>
              <input type="number" id="ie-inc-amount"
                placeholder="0" min="0"/>
            </div>
            <div class="field">
              <label>Branch</label>
              <select id="ie-inc-branch">
                <option value="الغردقة">Hurghada — الغردقة</option>
                <option value="الاسكندرية">Alexandria — الاسكندرية</option>
                <option value="القاهرة">Cairo — القاهرة</option>
                <option value="رشيد">Rashid — رشيد</option>
              </select>
            </div>
            <div class="field">
              <label>Collected By</label>
              <input type="text" id="ie-inc-by"
                placeholder="Employee name"
                value="${G.user?.username || ''}"/>
            </div>
            <div class="field">
              <label>Day</label>
              <input type="number" id="ie-inc-day"
                value="${dd}" min="1" max="31"/>
            </div>
            <div class="field">
              <label>Month</label>
              <input type="number" id="ie-inc-month"
                value="${mm}" min="1" max="12"/>
            </div>
            <div class="field">
              <label>Year</label>
      ...
      <div id="ie-car-fields" style="display:none;">
        <div class="panel">
          ...
          <div class="form-grid">
            <div class="field" style="grid-column:1/-1;">
              <label>Contract / Order No. *</label>
              <input type="text" id="ie-car-order"
            ...
            <div class="field">
              <label>Amount (EGP) *</label>
              <input type="number" id="ie-car-amount"
                placeholder="0" min="0"/>
            </div>
            <div class="field" style="grid-column:1/-1;">
              <label>Statement (بيان) *</label>
              <input type="text" id="ie-car-statement"
                placeholder="Describe the expense"/>
            </div>
            <div class="field">
              <label>Paid By</label>
              <input type="text" id="ie-car-by"
                placeholder="Employee name"
                value="${G.user?.username || ''}"/>
            </div>
            <div class="field">
              <label>Charged To (مستحق من)</label>
              <select id="ie-car-due-from">
                <option value="الشركة">Company (الشركة)</option>
                <option value="العميل">Client (العميل)</option>
                <option value="صاحب السيارة">Car Owner</option>
                <option value="التأمين">Insurance (التأمين)</option>
              </select>
            </div>
            <div class="field">
              <label>Branch</label>
            ...
            <div class="field">
              <label>Day</label>
              <input type="number" id="ie-car-day"
                value="${dd}" min="1" max="31"/>
            </div>
            <div class="field">
              <label>Month</label>
              <input type="number" id="ie-car-month"
                value="${mm}" min="1" max="12"/>
            </div>
            <div class="field">
              <label>Year</label>
          ...
          <button class="btn btn-danger btn-full mt-12"
            onclick="saveIECarExp()">
            💾 Save Car Expense
          </button>
        </div>
      </div>
      ...
      <div id="ie-gen-fields" style="display:none;">
        <div class="panel">
          ...
          <div class="form-grid">
            ...
            <div class="field">
              <label>Amount (EGP) *</label>
              <input type="number" id="ie-gen-amount"
                placeholder="0" min="0"/>
            </div>
            <div class="field" style="grid-column:1/-1;">
              <label>Statement (بيان المصروف) *</label>
            ...
            <div class="field">
              <label>Employee Name (if applicable)</label>
            ...
            <div class="field">
              <label>Day</label>
              <input type="number" id="ie-gen-day"
                value="${dd}" min="1" max="31"/>
            </div>
            <div class="field">
              <label>Month</label>
              <input type="number" id="ie-gen-month"
                value="${mm}" min="1" max="12"/>
            </div>
            <div class="field">
              <label>Year</label>
          ...
          <button class="btn btn-warning btn-full mt-12"
            onclick="saveIEGenExp()">
            💾 Save General Expense
          </button>
        </div>
      </div>

      <!-- Help when no type selected -->
...
window.saveIEIncome = async function() {
  ...
  if (!orderNo)   { toast('Order number is required for income entries', 'error'); return; }
  ...
  try {
    ...
    const branchEN = BRANCH_MAP[
      Object.keys(BRANCH_AR).find(k => BRANCH_AR[k] === branch) || ''
    ] || branch;
    ...
    await logAction('ADD', 'Income & Expenses',
      `Income: ${statement} | Order: ${orderNo} | £${amount} | ${branch}`);
...
window.saveIECarExp = async function() {
  ...
  if (!category)  { toast('Expense category is required', 'error'); return; }
  if (amount <= 0){ toast('Amount must be greater than 0', 'error'); return; }
  if (!statement) { toast('Statement is required', 'error'); return; }
...
window.saveIEGenExp = async function() {
  ...
  if (category === 'أخرى' && !otherDesc) {
    toast('"Other" requires a description', 'error'); return;
  }
  if (amount <= 0){ toast('Amount must be greater than 0', 'error'); return; }
  if (!statement) { toast('Statement is required', 'error'); return; }

  const finalStatement = category === 'أخرى' ? otherDesc : statement;
  const branchAR = BRANCH_AR[
    Object.keys(BRANCH_MAP).find(k => BRANCH_MAP[k] === branch) || ''
  ] || branch;
  ...
  try {
    ...
    await logAction('ADD', 'Income & Expenses',
      `General expense: ${category} | ${finalStatement} | £${amount} | ${branch}`);
...
window.exportIncomeCSV = function() {
  try {
    ...
    const headers = ['No','Date','Statement','Order Ref','Branch','Type',
                     'Collected By','Amount'];
    const csv = [
      headers.join(','),
      ...rows.map(r => {
...
// ============================================================
// OPEN ORDER FROM INCOME TABLE
// (reuses function from receipts.js if available)
// ============================================================

if (!window.openOrderByNo) {
