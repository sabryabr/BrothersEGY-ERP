    ...
    <div style="padding:16px 0 8px;">
      ...
      <div id="bulk-toolbar"
        ...
        <button class="btn btn-success btn-sm"
          onclick="bulkGenerateReceipts()">🧾 Bulk Receipts</button>
        <button class="btn btn-ghost btn-sm"
          onclick="bulkGenerateContracts('en')">📄 Bulk EN Contracts</button>
        <button class="btn btn-ghost btn-sm"
          onclick="bulkGenerateContracts('ar')">📝 Bulk AR Contracts</button>
...
window.subscribeOrders = function () {
  ...
  if (window._ordersUnsub) { window._ordersUnsub(); window._ordersUnsub = null; }
  ...
  const unsub = query.onSnapshot(snap => {
    ...
    filterOrders();
    updateOrderSummaryBar();
  }, e => { console.warn('Orders snapshot error:', e.message); });
...
window.filterOrders = debounce(function () {
  ...
  let filtered = allOrders.filter(o => {
    ...
    if (status !== 'all') {
      ...
      if      (status === 'overdue') { if (!isOverdueOrder) return false; }
      else if (status === 'Active')  { if (st !== 'Active' || isOverdueOrder) return false; }
      else if (status === 'Future')  { if (st !== 'Future') return false; }
...
window.clearFilter = function (key) {
  const map = { search:'ob-search', status:'ob-status', branch:'ob-branch', from:'ob-from', to:'ob-to' };
...
window.updateOrderSummaryBar = function () {
  ...
  const active   = allOrders.filter(o => {
    const st = getOrderStatus(o); const { end } = getOrderDates(o);
    return st === 'Active' && !(end && today > end && !o.closed);
  }).length;
  const overdue  = allOrders.filter(o => {
    const { end } = getOrderDates(o); const st = getOrderStatus(o);
  ...
  const closed   = allOrders.filter(o => {
    const st = getOrderStatus(o); return st === 'Closed' || o.closed === true;
  }).length;
  const future   = allOrders.filter(o => {
    const st = getOrderStatus(o); const { start } = getOrderDates(o);
...
function _buildOrderRow(o, today) {
  ...
  const days    = start && end ? Math.ceil((end - start) / 86400000) : parseInt(o.rental_days || 0);
  const total   = getOrderTotal(o);
  const paid    = getOrderPaid(o);
  const daily   = parseAmount(o['سعر السيارة اليومي بالجنيه المصري']) ||
                  (total / Math.max(1, parseFloat(o.rental_days || 1)));
  ...
  const statusKey   = isAccident ? 'accident' : isOverdue ? 'overdue' : isClosed ? 'closed'
                    : isFuture ? 'future' : (statusRaw || 'active').toLowerCase();
  const statusLabel = isAccident ? 'ACCIDENT' : isOverdue ? 'OVERDUE' : isClosed ? 'Closed'
                    : isFuture ? 'Future' : (statusRaw || 'Active');
  const statusColors = {
  ...
  const rowBg     = isAccident ? 'rgba(249,115,22,0.04)' : isOverdue ? 'rgba(239,68,68,0.04)' : 'transparent';
...
window.openOrderDetail = async function (orderId) {
  ...
  const bannerColor = isAccident ? 'var(--warning)' : isOverdue ? 'var(--danger)'
                    : isClosed   ? 'var(--text3)'   : isFuture  ? 'var(--accent)' : 'var(--success)';
  const bannerBg    = isAccident ? 'rgba(249,115,22,0.1)' : isOverdue ? 'rgba(239,68,68,0.1)'
                    : isClosed   ? 'rgba(100,116,139,0.1)': isFuture  ? 'rgba(59,130,246,0.1)' : 'rgba(34,197,94,0.1)';
  const statusLabel = isAccident ? '— ACCIDENT' : isOverdue ? '— OVERDUE' : isClosed ? '— CLOSED'
  ...
  const html = `
    ...
    <div style="background:var(--surface2);border-radius:10px;padding:12px;margin-bottom:12px;">
      ...
      <div style="display:flex;gap:16px;flex-wrap:wrap;font-size:12px;">
        ...
        <div><span style="color:var(--text3);">Duration:</span>
          ${o['rental_days'] || (start && end ? Math.ceil((end - start) / 86400000) : '-')} days</div>
        <div><span style="color:var(--text3);">Branch:</span>
          ${BRANCH_MAP[o['فرع الإصدار']] || o['فرع الإصدار'] || '-'}</div>
      </div>
    </div>
    ...
    <div style="background:var(--surface2);border-radius:10px;padding:12px;margin-bottom:12px;">
      ...
      <div style="margin-top:12px;padding:10px;border-radius:8px;text-align:center;
        ...
        <div style="font-size:18px;font-weight:900;color:${debt > 0 ? 'var(--danger)' : 'var(--success)'};">
    ...
    <div style="background:var(--surface2);border-radius:10px;padding:12px;margin-bottom:12px;">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;">
        <button class="btn btn-ghost btn-sm"
          onclick="addDepositPayment('${o.id}','collect')">💰 Collect Deposit</button>
        <button class="btn btn-ghost btn-sm"
          onclick="addDepositPayment('${o.id}','return')">↩️ Return Deposit</button>
      </div>
    </div>
...
window.openOrderEdit = async function (orderId) {
  ...
  openModal('Edit Order #' + (o['No.'] || orderId), html);
};
  ...
  if (!isPriv && order) {
    ...
    if (discountPct > threshold) {
      await db.collection('approvals').add({
        ...
        details         : 'Original: ' + fmtMoney(origRate) + ' -> Requested: ' + fmtMoney(newDaily) +
...
window.updateQPTotal = function () {
  ...
  if (el) el.textContent = fmtMoney(egp + (usd * G.ratesUSD) + (eur * G.ratesEUR) + card + online);
};
...
window.saveQuickPayment = async function (orderId, totalDue, prevPaid) {
  ...
  try {
    ...
    const upd = { 'المدفوع EGP': String(newPaidEGP), _sys_updated: Date.now() };
    ...
    const car     = G.fleet.find(c => String(c.ID || c.id) === String(order?.['كود السيارة'] || '')) || {};
    const methods = [];
...
window.generateSinglePaymentReceipt = async function (payment, paymentId, orderId) {
  try {
      ...
      <div style="font-family:'Courier New',monospace;width:80mm;padding:10px;
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;
          ...
          <div style="text-align:center;">
            <img src="https://api.qrserver.com/v1/create-qr-code/?data=${encodeURIComponent(verifyUrl)}&size=70x70"
              style="height:52px;">
        ...
        <div style="background:#f8fafc;border-radius:4px;padding:8px;margin-bottom:8px;text-align:center;">
          <div style="font-size:7pt;font-weight:700;color:#666;text-transform:uppercase;">
            This Payment Only
          </div>
          <div style="font-size:14pt;font-weight:900;">
            ${amountStr || fmtMoney(payment.total_egp_equiv || 0)}
          </div>
          <div style="font-size:7pt;color:#555;">${payment.payment_method || 'Cash'}</div>
          <div style="font-size:7pt;color:#777;">Category: ${payment.category || '—'}</div>
          ${payment.notes ? `<div style="font-size:7pt;font-style:italic;">Note: ${payment.notes}</div>` : ''}
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px;
          font-size:7pt;background:#f8fafc;border-radius:4px;padding:6px;margin-bottom:8px;">
          <div>
    ...
    const html = `<!DOCTYPE html><html><head>
      <meta charset="UTF-8"><title>Receipt ${rcRef}</title>
      <style>
        @media print { .no-print { display:none!important; } body { margin:0; } }
        body { font-family:'Courier New',monospace; background:#fff; padding:20px; }
        .btn-print { padding:10px 20px; background:#3b82f6; color:#fff; border:none;
...
window.reprintFromRecord = function (r) {
  ...
  const slip = copyType => `
    <div style="font-family:'Courier New',monospace;width:80mm;padding:10px;
      ...
      <div style="background:#f8fafc;border-radius:4px;padding:8px;margin-bottom:8px;text-align:center;">
        ...
        <div style="font-size:7pt;color:#555;">${r.payment_details || r.payment_method || '—'}</div>
        ${r.category ? `<div style="font-size:7pt;color:#777;">Category: ${r.category}</div>` : ''}
        ${r.notes    ? `<div style="font-size:7pt;font-style:italic;">Note: ${r.notes}</div>`  : ''}
      </div>
      <div style="font-size:6pt;color:#999;text-align:center;">
  ...
  const html = `<!DOCTYPE html><html><head>
    <meta charset="UTF-8"><title>Receipt Reprint ${r.receipt_ref || r.id}</title>
    <style>
      @media print { .no-print { display:none!important; } body { margin:0; } }
      body { font-family:'Courier New',monospace; background:#fff; padding:20px; }
      .btn-print { padding:10px 20px; background:#3b82f6; color:#fff; border:none;
    ...
    const html = `
      <div style="margin-bottom:16px;">
        ...
        ${payments.length ? `
          ...
          </table>` : `<div style="color:var(--text3);font-size:12px;">No payment log entries found.</div>`}
      </div>
      <div>
        ...
        ${receipts.length ? `
            ...
            <tbody>
              ${receipts.map(r => `
                <tr style="border-bottom:1px solid var(--border);">
                  ...
                  <td style="padding:6px;font-size:10px;">${r.issued_at || r.receipt_date || '—'}</td>
          ...
          </table>` : `<div style="color:var(--text3);font-size:12px;">No receipts found for this order.</div>`}
      </div>`;
...
window.editPayment = async function (paymentId, orderId) {
  ...
  } catch (e) { toast('Failed to load payment: ' + e.message, 'error'); return; }
  ...
  const html = `
    <div style="display:grid;gap:10px;">
      ...
      <div>
        <label style="font-size:11px;font-weight:700;color:var(--text3);">Payment Method</label>
      ...
      <div>
        ...
        <select id="ep-category"
          style="width:100%;margin-top:4px;padding:8px;background:var(--surface2);
            border:1px solid var(--border);border-radius:8px;color:var(--text);">
          ${['Rental Payment','Advance Payment','Full Settlement','Overdue Settlement',
             'Security Deposit','Other'].map(c =>
            `<option value="${c}" ${payment.category === c ? 'selected' : ''}>${c}</option>`
          ).join('')}
        </select>
      </div>
      <div>
      ...
      <div style="display:flex;gap:8px;margin-top:4px;">
        <button class="btn btn-success" style="flex:1;"
          onclick="savePaymentEdit('${paymentId}','${orderId}')">💾 Save Changes</button>
        <button class="btn btn-ghost" style="flex:1;"
          onclick="closeModal()">Cancel</button>
      </div>
    </div>`;
...
window.addDepositPayment = function (orderId, type) {
  ...
  const html = `
    ...
    <div style="margin-bottom:12px;">
      <label style="font-size:11px;font-weight:700;color:var(--text3);">Notes (optional)</label>
      <input type="text" id="dep-notes" placeholder="e.g. Cheque #1234"
        style="width:100%;margin-top:4px;padding:8px;background:var(--surface2);
          border:1px solid var(--border);border-radius:8px;color:var(--text);"/>
    </div>
...
// ============================================================
// BULK RECEIPTS GENERATOR  ← NEW
// ============================================================
window.bulkGenerateReceipts = async function () {
  const ids = [...window.bulkSelected];
  if (ids.length === 0) { toast('No orders selected', 'error'); return; }
  if (!confirm('Generate receipts for ' + ids.length + ' orders?\n\nEach receipt will open in a new tab.')) return;

  toast('⏳ Generating ' + ids.length + ' receipts...', 'info', 4000);

  let done = 0, failed = 0;
  for (const orderId of ids) {
    try {
      await generateOrderReceiptDirect(orderId);
      done++;
      // Small delay to avoid popup blocker triggering on rapid opens
      await new Promise(r => setTimeout(r, 350));
    } catch (e) {
      failed++;
      console.warn('Bulk receipt failed for ' + orderId + ':', e.message);
    }
  }

  toast('✅ Receipts done: ' + done + (failed > 0 ? ' | Failed: ' + failed : ''), 'success', 6000);
  await logAction('GENERATE', 'Order Book', 'Bulk receipts: ' + done + ' generated');
};

// ============================================================
// BULK CONTRACTS GENERATOR  ← NEW
// ============================================================
window.bulkGenerateContracts = async function (lang) {
  const ids = [...window.bulkSelected];
  if (ids.length === 0) { toast('No orders selected', 'error'); return; }

  const langLabel = lang === 'ar' ? 'Arabic' : 'English';
  if (!confirm('Generate ' + langLabel + ' contracts for ' + ids.length + ' orders?\n\nEach contract will open in a new tab.')) return;

  toast('⏳ Generating ' + ids.length + ' ' + langLabel + ' contracts...', 'info', 4000);

  let done = 0, failed = 0;
  for (const orderId of ids) {
    try {
      await generateOrderContractDirect(orderId, lang);
      done++;
      await new Promise(r => setTimeout(r, 400));
    } catch (e) {
      failed++;
      console.warn('Bulk contract failed for ' + orderId + ':', e.message);
    }
  }

  toast('✅ Contracts done: ' + done + (failed > 0 ? ' | Failed: ' + failed : ''), 'success', 6000);
  await logAction('GENERATE', 'Order Book', 'Bulk ' + langLabel + ' contracts: ' + done + ' generated');
};
    ...
    const slip = copyType => `
      <div style="font-family:'Courier New',monospace;width:80mm;padding:10px;
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;
          ...
          <div style="text-align:center;">
            <img src="https://api.qrserver.com/v1/create-qr-code/?data=${encodeURIComponent(verifyUrl)}&size=60x60"
              style="height:50px;">
